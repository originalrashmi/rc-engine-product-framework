...
════════════════════════════════════════════════════════════════════════════════
  ...
  ACTIVE MODULES:
    ${state.config.activeModules.map((m) => `[x]  ${m}`).join('\n    ')}
  ...
  ${
    ...
    Total:     ${lastScan.summary.totalFindings}`
      : 'No scans yet. Run postrc_scan to validate.'
  }
${tokenTracker.getDomainSummary('post-rc')}${formatTokenReport()}${formatCostSummary()}${getLearningSummary()}${formatRecentActivity(project_path)}
