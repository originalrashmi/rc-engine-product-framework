...
const APP_PATTERNS: AppPatternRule[] = [
  ...
  },
  {
    ...
    category: 'auth-abuse',
    remediation:
      'Return the same generic message for both invalid email and invalid password: "Invalid credentials".',
  },
  {
  ...
  },
  {
    ...
    cweId: 'CWE-521',
    category: 'auth-abuse',
    remediation: 'Enforce minimum 8-character passwords. Consider NIST guidelines (8+ chars, check against breach lists).',
  },

  // ── Skill 2: Authorization + RLS ──
  {
    pattern: /\.(?:findMany|findFirst|findUnique|select|from)\s*\([^)]*\)(?![^}]*(?:where|filter).*(?:userId|user_id|ownerId|owner_id|createdBy|created_by))/gis,
    skill: 2,
  ...
  // ── Skill 3: Validation + Input Safety ──
  {
    ...
    remediation:
      'Validate all request body fields with Zod, Yup, or joi before use. Parse, don\'t validate.',
  },
  {
    ...
    category: 'validation-input',
    remediation:
      'Sanitize HTML with DOMPurify before rendering. Prefer text content or safe templating.',
  },
  {
    ...
    category: 'validation-input',
    remediation:
      'Explicitly pick allowed fields from the request body. Never pass raw input to ORM create/update.',
  },

  // ── Skill 4: Search + Query Safety ──
  {
    pattern: /(?:limit|take|pageSize|page_size)\s*[:=]\s*(?:req\.|query\.|params\.|parseInt\s*\(\s*(?:req|query|params))/gi,
    skill: 4,
    ...
    remediation:
      'Cap pagination limits server-side (e.g., Math.min(userLimit, 100)). Never allow unbounded queries.',
  },
  {
    ...
    category: 'search-query',
    remediation:
      'Validate sort fields against an allowlist of permitted column names. Reject unknown fields.',
  },
  {
...
async function runStructuralAnalysis(
  ...
  if (activeSkills.has(1)) {
    ...
    if (hasSignup && !hasCaptcha) {
      findings.push({
        ...
        description:
          'A signup/registration flow exists but no CAPTCHA library is referenced. Bot abuse risk.',
        remediation:
          'Add Cloudflare Turnstile or reCAPTCHA to registration forms to prevent automated signups.',
        category: 'auth-abuse',
      });
    }
  }
  ...
  if (activeSkills.has(2)) {
    ...
    if (hasApiRoutes && !hasMiddleware) {
      findings.push({
        ...
        remediation:
          'Create auth middleware that validates session/JWT on all protected API routes.',
        category: 'authorization-rls',
      });
    }
  }
  ...
  if (activeSkills.has(3)) {
    ...
    const hasApiEndpoints = /(?:export\s+(?:async\s+)?function\s+(?:GET|POST|PUT|PATCH|DELETE)|app\.(?:get|post|put|patch|delete)|router\.(?:get|post|put|patch|delete))/i.test(code);
  ...
  if (activeSkills.has(4)) {
    ...
    if (hasDbQueries && !hasPagination) {
      findings.push({
        ...
        remediation:
          'Add take/limit with a server-side cap (e.g., max 100) to all list queries.',
        category: 'search-query',
      });
    }
  }
