...
export const DesignIntakeInputSchema = z.object({
  ...
  brandGuidelinesUrl: z
    .string()
    .optional()
    .describe('URL or path to existing brand guidelines document'),
  brandPersonality: z
    .array(z.string())
    .optional()
    .describe('3-5 brand personality traits: "professional", "playful", "luxurious", "bold", "technical", "warm", "minimal"'),
  existingLogoPath: z
    .string()
    .optional()
    .describe('Path to existing logo file for context'),
  ...
  // ── Accessibility ─────────────────────────────────────────────────────────
  wcagTarget: z
    .enum(['A', 'AA', 'AAA', 'no-preference'])
    .optional()
    .describe('WCAG compliance target level'),
  ...
  competitorLikes: z
    .array(z.string())
    .optional()
    .describe('Specific things the user likes about competitor designs'),
  ...
  priorityScreens: z
    .array(z.string())
    .optional()
    .describe('Which screens matter most for design quality (top 3)'),
...
export interface DesignDirectionFinding {
  category: 'color' | 'typography' | 'layout' | 'style' | 'competitive' | 'interaction' | 'component' | 'platform' | 'accessibility' | 'mood' | 'screen-inventory';
