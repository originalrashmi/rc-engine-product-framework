...
import type { ChallengeInput, ChallengeFinding, ChallengeLensResult, ChallengerLens } from '../challenger-types.js';
...
export class ChallengerAgent extends BaseAgent {
  ...
  async challenge(state: ProjectState, input: ChallengeInput): Promise<AgentResult> {
    ...
    const lensResults = await Promise.all(lenses.map((lens) => this.runLens(lens, context, state)));
  ...
  private async runLens(lens: ChallengerLens, context: string, state: ProjectState): Promise<ChallengeLensResult> {
...
Focus: Contrast, keyboard, screen reader, motion, cognitive a11y, responsive.`,
    ...
    if (budget > 0) {
      ...
      if (fs.existsSync(designDir)) {
        ...
        const designFiles = fs
          ...
          .filter((f) => (f.endsWith('.md') || f.endsWith('.json')) && f !== 'CHALLENGE-REPORT.md');
        ...
        if (budget > 0) {
          ...
          for (const subdir of subdirs) {
            ...
            const htmlFiles = fs.readdirSync(subdirPath).filter((f) => f.endsWith('.html'));
   ...
   */
  static buildIterationFeedback(criticalIssues: ChallengeFinding[], highIssues: ChallengeFinding[]): string {
    const lines: string[] = [];
