...
export class DesignIntakeAgent extends BaseAgent {
  ...
  async analyze(
    ...
    const hasAnyPreferences = !!(
      ...
      input.aestheticDirection ||
      input.brandPersonality?.length ||
      input.navigationPattern ||
      input.contentDensity ||
      input.animationPreference ||
      input.interactionDensity ||
      input.componentPreferences ||
      input.iconStyle ||
      input.imageryStyle ||
      input.primaryPlatform ||
      input.devicePriority ||
      input.designSystemFramework ||
      input.wcagTarget ||
      input.accessibilityRequirements?.length ||
      input.competitorLikes?.length ||
      input.competitorDislikes?.length ||
      input.keyScreens?.length ||
      input.criticalFlows?.length ||
      input.priorityScreens?.length ||
      input.structuralPreferences?.length ||
    ...
    const assessment = await this.evaluateDirection(input, prdContext, icpData, brandProfile, competitiveLandscape, [
      ...referenceFindings,
      ...fontFindings,
    ]);
...
${urls.map((u, i) => `${i + 1}. ${u}`).join('\n')}`;
  ...
  private async analyzeReferences(input: DesignIntakeInput, icpData?: string): Promise<DesignDirectionFinding[]> {
  ...
  private async validateFontPreferences(fonts: string[]): Promise<DesignDirectionFinding[]> {
    const findings: DesignDirectionFinding[] = [];
  ...
  private async evaluateDirection(
    ...
    if (input.colorPreferences?.disliked)
      userPrefs.push(`Colors disliked: ${input.colorPreferences.disliked.join(', ')}`);
    if (input.colorPreferences?.semanticRequirements) {
      const sem = input.colorPreferences.semanticRequirements;
      const parts = [
        sem.success && `success=${sem.success}`,
        sem.warning && `warning=${sem.warning}`,
        sem.error && `error=${sem.error}`,
        sem.info && `info=${sem.info}`,
      ].filter(Boolean);
    ...
    if (input.navigationPattern && input.navigationPattern !== 'no-preference')
      userPrefs.push(`Navigation: ${input.navigationPattern}`);
    ...
    if (input.aestheticDirection && input.aestheticDirection !== 'no-preference')
      userPrefs.push(`Aesthetic: ${input.aestheticDirection}`);

    // Interaction & motion
    if (input.animationPreference && input.animationPreference !== 'no-preference')
      userPrefs.push(`Animation: ${input.animationPreference}`);
    ...
    if (input.devicePriority && input.devicePriority !== 'no-preference')
      userPrefs.push(`Device priority: ${input.devicePriority}`);
    if (input.designSystemFramework && input.designSystemFramework !== 'no-preference')
      userPrefs.push(`Design system: ${input.designSystemFramework}`);

    // Accessibility
    if (input.wcagTarget && input.wcagTarget !== 'no-preference') userPrefs.push(`WCAG target: ${input.wcagTarget}`);
    if (input.accessibilityRequirements?.length)
      userPrefs.push(`A11y requirements: ${input.accessibilityRequirements.join(', ')}`);
...
- Any open questions`;
  ...
  private parseCompetitiveLandscape(text: string, competitorCount: number): CompetitiveLandscape {
  ...
  private parseAssessment(
    ...
    if (
      explicitVerdict === 'proceed' ||
      explicitVerdict === 'proceed_with_adjustments' ||
      explicitVerdict === 'reconsider'
    ) {
    ...
    const extractedConstraints: ExtractedDesignConstraints = {
      ...
      moodDirection: {
        ...
        rationale:
          this.extractSectionFromText(text, 'mood') ?? this.extractSectionFromText(text, 'aesthetic') ?? 'See analysis',
      },
      interactionDirection: {
        ...
        rationale:
          this.extractSectionFromText(text, 'interaction') ??
          this.extractSectionFromText(text, 'animation') ??
          'See analysis',
      },
      componentDirection: {
        cardStyle:
          input.componentPreferences?.cardStyle !== 'no-preference' ? input.componentPreferences?.cardStyle : undefined,
        formStyle:
          input.componentPreferences?.formStyle !== 'no-preference' ? input.componentPreferences?.formStyle : undefined,
        buttonStyle:
          input.componentPreferences?.buttonStyle !== 'no-preference'
            ? input.componentPreferences?.buttonStyle
            : undefined,
        modalPreference:
          input.componentPreferences?.modalPreference !== 'no-preference'
            ? input.componentPreferences?.modalPreference
            : undefined,
      ...
      platformDirection: {
        ...
        designSystemFramework:
          input.designSystemFramework !== 'no-preference' ? input.designSystemFramework : undefined,
  ...
  private formatAssessment(assessment: DesignDirectionAssessment, isAutonomous: boolean): string {
    const lines: string[] = [];
    ...
    if (assessment.findings.length > 0) {
      ...
      for (const f of assessment.findings) {
        const alignIcon = f.alignment === 'aligned' ? '[OK]' : f.alignment === 'misaligned' ? '[!!]' : '[--]';
    ...
    if (ec.typographyDirection.pairingSuggestions.length > 0)
      lines.push(`  Pairings: ${ec.typographyDirection.pairingSuggestions.join(', ')}`);
    lines.push('');
    ...
    if (ec.interactionDirection.interactionDensity)
      lines.push(`  Density: ${ec.interactionDirection.interactionDensity}`);
    lines.push('');
    ...
    if (ec.platformDirection.designSystemFramework)
      lines.push(`  Framework: ${ec.platformDirection.designSystemFramework}`);
    lines.push('');
    ...
    if (ec.accessibilityDirection.requirements.length > 0)
      lines.push(`  Requirements: ${ec.accessibilityDirection.requirements.join(', ')}`);
    lines.push('');

    if (ec.screenInventory.keyScreens.length > 0) {
      lines.push('### Screen Inventory');
      lines.push(
        `**Key screens (${ec.screenInventory.keyScreens.length}):** ${ec.screenInventory.keyScreens.join(', ')}`,
      );
      if (ec.screenInventory.criticalFlows.length > 0)
        lines.push(`**Critical flows:** ${ec.screenInventory.criticalFlows.join('; ')}`);
      if (ec.screenInventory.priorityScreens.length > 0)
        lines.push(`**Priority screens:** ${ec.screenInventory.priorityScreens.join(', ')}`);
      lines.push('');
    }
  ...
  private mapDescriptionToMood(
    ...
    if (desc.includes('editorial') || desc.includes('luxury') || desc.includes('premium')) return 'luxurious';
    if (desc.includes('playful') || desc.includes('fun') || desc.includes('friendly')) return 'playful';
    if (desc.includes('technical') || desc.includes('developer') || desc.includes('code')) return 'technical';
    if (desc.includes('bold') || desc.includes('brutalist') || desc.includes('strong')) return 'bold';
    if (desc.includes('warm') || desc.includes('cozy') || desc.includes('inviting')) return 'warm';
    if (desc.includes('minimal') || desc.includes('clean') || desc.includes('simple')) return 'minimal';
