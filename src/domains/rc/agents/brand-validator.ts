...
export class BrandComplianceChecker {
   ...
   */
  check(profile: BrandProfile, designOutput: DesignOutputForValidation): BrandComplianceResult {
  ...
  private checkColors(profile: BrandProfile, output: DesignOutputForValidation): BrandComplianceCheck[] {
    const checks: BrandComplianceCheck[] = [];
    ...
    if (output.colorsUsed) {
      ...
      for (const color of output.colorsUsed) {
        const isInBrand = brandColors.some((bc) => bc.toLowerCase() === color.toLowerCase());
        const isTintShade = brandColors.some((bc) => this.isTintOrShade(bc, color));
      ...
      if (output.ctaColors) {
        const primaryUsed = output.ctaColors.some((c) => c.toLowerCase() === profile.colors.primary.hex.toLowerCase());
        if (!primaryUsed) {
          checks.push({
            category: 'color',
  ...
  private checkTypography(profile: BrandProfile, output: DesignOutputForValidation): BrandComplianceCheck[] {
    const checks: BrandComplianceCheck[] = [];
  ...
  private checkShape(profile: BrandProfile, output: DesignOutputForValidation): BrandComplianceCheck[] {
    const checks: BrandComplianceCheck[] = [];
  ...
  private checkVoice(profile: BrandProfile, output: DesignOutputForValidation): BrandComplianceCheck[] {
    const checks: BrandComplianceCheck[] = [];
  ...
  private checkLogo(profile: BrandProfile, _output: DesignOutputForValidation): BrandComplianceCheck[] {
    const checks: BrandComplianceCheck[] = [];
  ...
  private checkAccessibility(profile: BrandProfile, _output: DesignOutputForValidation): BrandComplianceCheck[] {
    const checks: BrandComplianceCheck[] = [];
