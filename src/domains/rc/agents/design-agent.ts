...
export class DesignAgent extends BaseAgent {
  ...
  async generate(state: ProjectState, input: DesignInput): Promise<AgentResult> {
    ...
    for (const option of spec.options) {
      try {
        ...
        const hasContent = screens.some(
          (s) => s.hifiHtml.length > 100 && !s.hifiHtml.includes('wireframe not generated'),
        );
        if (!hasContent) {
  ...
  private validateConstraints(spec: DesignSpec, input: DesignInput): string[] {
    ...
    if (input.brandProfilePath) {
      try {
        ...
        /* continue without brand validation */
      }
    }
    ...
    if (input.designIntakePath) {
      try {
            ...
            for (const option of spec.options) {
              ...
              for (const avoided of constraints.colorDirection.avoid) {
                ...
                if (paletteColors.includes(avoidedUpper)) {
                  warnings.push(`Option ${option.id}: Uses avoided color ${avoided} (from intake preferences).`);
                }
              }
            }
          }
        ...
        /* continue without intake validation */
      }
    }
  ...
  private async selfCritique(state: ProjectState, spec: DesignSpec, input?: DesignInput): Promise<string | null> {
    ...
    const specSummary = spec.options
      .map((o) => {
...
If the design is solid, say so briefly.`;
  ...
  private formatIntakeConstraints(constraints: ExtractedDesignConstraints): string {
    ...
    if (constraints.colorDirection.primary || constraints.colorDirection.palette?.length) {
      ...
      if (constraints.colorDirection.palette?.length)
        sections.push(`- Palette: ${constraints.colorDirection.palette.join(', ')}`);
      if (constraints.colorDirection.avoid?.length)
        sections.push(`- AVOID: ${constraints.colorDirection.avoid.join(', ')}`);
      if (constraints.colorDirection.semanticColors) {
        const sc = constraints.colorDirection.semanticColors;
        sections.push(
          `- Semantic: success=${sc.success ?? 'auto'}, warning=${sc.warning ?? 'auto'}, error=${sc.error ?? 'auto'}, info=${sc.info ?? 'auto'}`,
        );
      }
    ...
    if (constraints.typographyDirection.headingStyle || constraints.typographyDirection.bodyStyle) {
      ...
      if (constraints.typographyDirection.headingStyle)
        sections.push(`- Heading style: ${constraints.typographyDirection.headingStyle}`);
      if (constraints.typographyDirection.bodyStyle)
        sections.push(`- Body style: ${constraints.typographyDirection.bodyStyle}`);
      if (constraints.typographyDirection.pairingSuggestions.length)
        sections.push(`- Pairings: ${constraints.typographyDirection.pairingSuggestions.join('; ')}`);
    }

    // Layout
    sections.push(`\n### Layout Direction`);
    if (constraints.layoutDirection.patterns.length)
      sections.push(`- Preferred patterns: ${constraints.layoutDirection.patterns.join(', ')}`);
    if (constraints.layoutDirection.avoidPatterns.length)
      sections.push(`- AVOID patterns: ${constraints.layoutDirection.avoidPatterns.join(', ')}`);
    if (constraints.layoutDirection.navigationPattern)
      sections.push(`- Navigation: ${constraints.layoutDirection.navigationPattern}`);
    if (constraints.layoutDirection.contentDensity)
      sections.push(`- Content density: ${constraints.layoutDirection.contentDensity}`);
    ...
    if (constraints.interactionDirection.animationLevel || constraints.interactionDirection.interactionDensity) {
      sections.push(`\n### Interaction & Motion`);
      if (constraints.interactionDirection.animationLevel)
        sections.push(`- Animation level: ${constraints.interactionDirection.animationLevel}`);
      if (constraints.interactionDirection.interactionDensity)
        sections.push(`- Interaction density: ${constraints.interactionDirection.interactionDensity}`);
    }
    ...
    if (constraints.platformDirection.primaryPlatform || constraints.platformDirection.designSystemFramework) {
      sections.push(`\n### Platform & Device`);
      if (constraints.platformDirection.primaryPlatform)
        sections.push(`- Platform: ${constraints.platformDirection.primaryPlatform}`);
      if (constraints.platformDirection.devicePriority)
        sections.push(`- Device priority: ${constraints.platformDirection.devicePriority}`);
      if (constraints.platformDirection.designSystemFramework)
        sections.push(`- Framework: ${constraints.platformDirection.designSystemFramework}`);
    }
    ...
    if (constraints.accessibilityDirection.wcagTarget || constraints.accessibilityDirection.requirements.length) {
      sections.push(`\n### Accessibility`);
      if (constraints.accessibilityDirection.wcagTarget)
        sections.push(`- WCAG target: ${constraints.accessibilityDirection.wcagTarget}`);
      if (constraints.accessibilityDirection.requirements.length)
        sections.push(`- Requirements: ${constraints.accessibilityDirection.requirements.join(', ')}`);
    }
    ...
    if (constraints.screenInventory.keyScreens.length) {
      ...
      if (constraints.screenInventory.priorityScreens.length)
        sections.push(
          `- Priority screens (design these first): ${constraints.screenInventory.priorityScreens.join(', ')}`,
        );
      if (constraints.screenInventory.criticalFlows.length)
        sections.push(`- Critical flows: ${constraints.screenInventory.criticalFlows.join('; ')}`);
    }
  ...
  private async generateWireframes(
    ...
    if (input?.copySystemPath) {
      try {
        ...
        /* continue with placeholder copy */
      }
    }
    ...
    if (input) {
      ...
      if (intakeConstraints) {
        ...
        if (intakeConstraints.accessibilityDirection.wcagTarget) {
          parts.push(
            `- WCAG target: ${intakeConstraints.accessibilityDirection.wcagTarget} - ensure contrast ratios meet this level`,
          );
        }
...
===END_WIREFRAME===`;
  ...
  private saveDesignArtifacts(state: ProjectState, spec: DesignSpec, wireframes: DesignWireframe[]): string[] {
    ...
    fs.writeFileSync(
      manifestPath,
      JSON.stringify({ generatedAt: new Date().toISOString(), wireframes: manifest }, null, 2),
      'utf-8',
    );
  ...
  async iterate(state: ProjectState, input: DesignIterateInput): Promise<AgentResult> {
    ...
    if (!option) {
      return {
        text: `Error: Design option "${optionId}" not found in spec.`,
        isError: true,
    ...
    const screens = input.targetScreens
      ? option.keyScreens.filter((s) =>
          input.targetScreens!.some((t) => s.name.toLowerCase().includes(t.toLowerCase())),
        )
      : option.keyScreens;

    if (screens.length === 0) {
      return {
        text: `Error: No matching screens found for targets: ${input.targetScreens?.join(', ')}`,
        isError: true,
      };
    }
...
===END_WIREFRAME===`;
    ...
    const text = await this.execute(
      ['skills/design/rc-design-patterns.md', 'skills/design/rc-design-accessibility.md'],
      instructions,
