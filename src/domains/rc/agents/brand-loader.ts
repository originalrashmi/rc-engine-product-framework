...
export class BrandAssetLoader {
   ...
   */
  private scanProjectFiles(projectPath: string): { profile: Partial<BrandProfile> | null; sources: string[] } {
    ...
    for (const configFile of configPaths) {
      ...
      try {
        ...
        if (Object.keys(extractedColors).length > 0) {
          ...
          const primary = extractedColors['primary'] ?? extractedColors['brand'] ?? Object.values(extractedColors)[0];
          const secondary =
            extractedColors['secondary'] ?? extractedColors['accent'] ?? Object.values(extractedColors)[1];
        ...
        const fontMatches = content.matchAll(/(?:heading|sans|serif|mono|body|accent)['"]?:\s*\[?['"]([^'"]+)['"]/g);
  ...
  private parseCssVariables(projectPath: string): Partial<BrandProfile> | null {
    const cssPaths = ['src/app/globals.css', 'src/styles/globals.css', 'styles/globals.css', 'app/globals.css'];
    ...
    for (const cssFile of cssPaths) {
      ...
      try {
        ...
        if (Object.keys(vars).length > 0) {
          const primary = vars['primary'] ?? vars['color-primary'] ?? vars['brand-primary'];
        ...
        const fontMatches = content.matchAll(/--font-([a-z]+):\s*['"]?([^;'"]+)['"]?;/gi);
  ...
  private detectFramework(projectPath: string): { framework?: string; componentLibrary?: string } | null {
    const pkgPath = path.join(projectPath, 'package.json');
    if (!fs.existsSync(pkgPath)) return null;

    try {
      const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8')) as Record<string, Record<string, string>>;
      ...
      if (deps['@radix-ui/react-dialog'] || deps['@radix-ui/themes']) result.componentLibrary = 'Radix';
      // shadcn is detected by the presence of components.json
      if (fs.existsSync(path.join(projectPath, 'components.json'))) result.componentLibrary = 'shadcn/ui';
  ...
  private parseConstantsDir(projectPath: string): Partial<BrandProfile> | null {
    ...
    for (const dir of constantsDirs) {
      ...
      try {
        ...
        const colorFile = files.find((f) => f.includes('color') && (f.endsWith('.ts') || f.endsWith('.js')));
  ...
  private detectLogo(projectPath: string): BrandProfile['logo'] | undefined {
    ...
    try {
      ...
      const logoFile = files.find(
        (f) => f.startsWith('logo') && (f.endsWith('.svg') || f.endsWith('.png') || f.endsWith('.jpg')),
      );
  ...
  private async scrapeUrl(url: string): Promise<Partial<BrandProfile> | null> {
    try {
      console.error(`[BrandAssetLoader] Scraping ${url} for brand signals...`);
      ...
      if (!response.ok) {
        console.error(`[BrandAssetLoader] HTTP ${response.status} from ${url}`);
        return null;
      }
      ...
      if (Object.keys(profile).length === 0) {
        console.error(`[BrandAssetLoader] No brand signals found in ${url}`);
        return null;
      }

      console.error(`[BrandAssetLoader] Extracted from ${url}: ${Object.keys(profile).join(', ')}`);
      return profile;
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      console.error(`[BrandAssetLoader] URL scrape failed for ${url}: ${message}`);
      return null;
    }
  }
   ...
   */
  private extractColorsFromHtml(html: string): Partial<BrandProfile>['colors'] | null {
   ...
   */
  private extractTypographyFromHtml(html: string): Partial<BrandProfile>['typography'] | null {
    const fonts: string[] = [];

    // Extract from Google Fonts <link> tags
    const googleFontsPattern = /fonts\.googleapis\.com\/css2?\?[^"'>\s]*family=([^"'>\s&]+)/gi;
   ...
   */
  private inferFontSource(fontFamily: string, html: string): 'google' | 'custom' | 'system' {
   ...
   */
  private mergePartial(target: Partial<BrandProfile>, source: Partial<BrandProfile>): void {
    this.deepMergeGaps(target as Record<string, unknown>, source as Record<string, unknown>);
  }

  private deepMergeGaps(target: Record<string, unknown>, source: Record<string, unknown>): void {
    for (const [key, value] of Object.entries(source)) {
      ...
      if (existing === undefined || existing === null) {
        ...
        this.deepMergeGaps(existing as Record<string, unknown>, value as Record<string, unknown>);
      }
