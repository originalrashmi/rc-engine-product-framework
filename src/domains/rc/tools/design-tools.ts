...
import type { CopyGenerateInput } from '../copy-types.js';
...
export function registerDesignTools(server: McpServer): void {
    ...
    'design_intake',
    {
      ...
      inputSchema: {
        ...
        layout_preferences: z
          .array(z.string())
          .optional()
          .describe('Layout preferences: "sidebar navigation", "card grid", "single-page app"'),
        navigation_pattern: z
          .enum(['top-nav', 'sidebar', 'bottom-tabs', 'hamburger', 'hybrid', 'no-preference'])
          .optional()
          .describe('Primary navigation pattern'),
        content_density: z
        ...
        mood_keywords: z
          .array(z.string())
          .optional()
          .describe('Design mood: "clean and modern", "warm and approachable", "bold and energetic"'),
        ...
        // Interaction & Motion
        animation_preference: z
          .enum(['none', 'subtle', 'moderate', 'expressive', 'no-preference'])
          .optional()
          .describe('Level of animation and micro-interactions'),
        interaction_density: z
          .enum(['spacious', 'balanced', 'compact'])
          .optional()
          .describe('Spacious (consumer) vs compact (power user)'),

        // Component Preferences
        card_style: z
          .enum(['elevated', 'outlined', 'flat', 'no-preference'])
          .optional()
          .describe('Card component style'),
        form_style: z
          .enum(['floating-label', 'outlined', 'filled', 'underlined', 'no-preference'])
          .optional()
          .describe('Form input style'),
        button_style: z.enum(['rounded', 'pill', 'square', 'no-preference']).optional().describe('Button style'),
        modal_preference: z
          .enum(['modal', 'inline', 'drawer', 'no-preference'])
          .optional()
          .describe('Overlay interaction pattern'),
        icon_style: z
        ...
        accessibility_requirements: z
          .array(z.string())
          .optional()
          .describe('Specific a11y needs: "screen reader support", "reduced motion", "high contrast"'),
        ...
        key_screens: z
          .array(z.string())
          .optional()
          .describe('Key screens: "landing page", "dashboard", "settings", "onboarding"'),
        ...
        if (prdContext.includes('No PRD found') || prdContext.includes('Could not load PRD')) {
          return {
            content: [
              {
                type: 'text' as const,
                text: `Error: No PRD found at ${args.project_path}. Design intake requires a PRD to evaluate design preferences against product requirements. Run rc_define or rc_import_prerc first.`,
              },
            ],
            isError: true,
          };
  ...
  server.registerTool(
    'brand_import',
    {
      ...
      inputSchema: {
        project_path: z.string().describe('Absolute path to the project directory'),
        website_url: z.string().optional().describe('URL of existing website/product to scrape for brand assets'),
        mode: z
          .enum(['strict', 'infer'])
          .optional()
  ...
  server.registerTool(
    ...
    async ({ project_path, option_id }) => {
      try {
        ...
        if (!option) {
          ...
          return {
            content: [
              {
                type: 'text' as const,
                text: `Error: Option "${option_id}" not found. Available options: ${available}`,
              },
            ],
            isError: true,
          };
        ...
        getOrchestrator().designSelect(project_path, option_id, specPath);
        return {
          content: [
            {
              type: 'text' as const,
              text: `Design option ${option_id} ("${option.name}") selected.\n\nNext steps:\n- "design_iterate" to refine wireframes\n- "design_challenge" to run design review\n- "rc_forge_task" to start building`,
            },
          ],
        };
      } catch (err) {
  ...
  server.registerTool(
    'design_pipeline',
    {
      ...
      inputSchema: {
        project_path: z.string().describe('Absolute path to the project directory'),
        option_count: z.enum(['1', '3']).optional().describe('Number of design options to generate (default: 3)'),
        inspiration: z.string().optional().describe('Design inspiration, references, or preferences'),
        run_challenge: z.boolean().optional().describe('Run the Design Challenger after generation (default: true)'),
    ...
    async ({
      ...
      try {
        ...
        try {
          ...
          steps.push(
            `Design Intake complete - verdict: ${intakeResult.text.match(/Verdict:\s*(\w+)/)?.[1] ?? 'see report'}`,
          );
        ...
        // Step 4.5: Present design options for user selection (human-in-the-loop)
        const specPath = path.join(project_path, 'rc-method', 'design', 'DESIGN-SPEC.json');
        if (fs.existsSync(specPath)) {
          try {
            const spec = JSON.parse(fs.readFileSync(specPath, 'utf-8'));
            ...
            if (options.length > 0) {
              steps.push('\n---\n\n## Design Options - Your Selection Required\n');
              for (const opt of options) {
                const isRec = opt.id === recId;
                steps.push(`### Option ${opt.id}: ${opt.name}${isRec ? ' (Recommended)' : ''}`);
                if (opt.description) steps.push(opt.description);
                // Show wireframe file paths for preview
                const wireframePath = path.join(
                  project_path,
                  'rc-method',
                  'design',
                  'wireframes',
                  `option-${opt.id.toLowerCase()}`,
                );
                if (fs.existsSync(wireframePath)) {
                  steps.push(`Wireframes: \`${wireframePath}/\``);
                }
                steps.push('');
              }
              steps.push(
                '**Action required:** Review the options above, then call `design_select` with your chosen option ID (e.g., "A", "B", or "C").',
              );
              steps.push('Do NOT proceed to architecture until you have selected a design option.');
            }
          } catch {
