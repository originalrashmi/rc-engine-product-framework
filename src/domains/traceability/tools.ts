...
export function registerTraceabilityTools(server: McpServer): void {
  ...
  server.tool(
    'trace_status',
    '[Pro+] Check traceability coverage. Read-only, safe to call anytime after trace_enhance_prd. Returns: total requirements, implemented count, verified count, coverage percentages, orphan lists. Use when user asks "what percentage is done?" or "what did we miss?" Also useful mid-build to track progress against requirements.',
